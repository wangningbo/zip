...
public class WZipOutputStream extends DeflaterOutputStream implements ZipConstants, ZipConstants64 {
	protected WZipEntry current;
	protected List<WZipEntry> entries = new ArrayList<>();
	...
	private static int version(WZipEntry e) throws ZipException {
		switch (e.method) {
		case DEFLATED:
			return 20;
		case STORED:
	 ...
	 */
	public static final int STORED = WZipEntry.STORED;

	/**
	 * Compression method for compressed (DEFLATED) entries.
	 */
	public static final int DEFLATED = WZipEntry.DEFLATED;
	 ...
	 */
	public WZipOutputStream(OutputStream out) {
	 ...
	 * @since 1.7
	 */
	public WZipOutputStream(OutputStream out, Charset charset) {
	...
	public WZipOutputStream(OutputStream out, List<WZipEntry> entries) {
		this(out);
		this.entries.addAll(entries);
		for (WZipEntry e : entries) {
			if (e.esize < 0) {
				throw new IllegalArgumentException("esize非法：" + e.esize);
			}
			written += e.esize;
		}
	}

	public List<WZipEntry> getEntries() {
		return this.entries;
	}
	 ...
	 */
	private void writeLOC(WZipEntry e) throws IOException {
		int flag = e.flag;
	...
	private void writeCEN(List<WZipEntry> entries) throws IOException {
		long offset = 0;
		for (WZipEntry e : entries) {
			writeCEN(e, offset);
			offset += e.esize;
		}
		writeEND(offset, written - offset);
	}

	/*
	 * Writes extra data descriptor (EXT) for specified entry.
	 */
	private void writeEXT(WZipEntry e) throws IOException {
	 ...
	 */
	private void writeCEN(WZipEntry e, long offset) throws IOException {
		int flag = e.flag;
	...
	private void writeEND(long off, long len) throws IOException {
		...
		if (hasZip64) {
			...
			writeLong(entries.size()); // number of directory entires on disk
			writeLong(entries.size()); // number of directory entires
