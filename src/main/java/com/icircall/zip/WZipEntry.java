 ...
 */
public class WZipEntry implements ZipConstants, Cloneable {
	...
	public void writeTo(DataOutputStream dataOut) throws IOException {
		...
		dataOut.writeInt(method);
		dataOut.writeInt(flag);
		dataOut.writeInt(extra == null ? 0 : extra.length);
		if (extra != null) {
			dataOut.write(extra);
		}
		dataOut.writeBoolean(comment != null);
		if (comment != null) {
			dataOut.writeUTF(comment);
		}
		dataOut.writeLong(esize);
	}

	public void readFrom(DataInputStream dataIn) throws IOException {
		this.name = dataIn.readUTF();
		this.time = dataIn.readLong();
		this.crc = dataIn.readLong();
		this.size = dataIn.readLong();
		this.csize = dataIn.readLong();
		this.method = dataIn.readInt();
		this.flag = dataIn.readInt();
		this.extra = new byte[dataIn.readInt()];
		if (this.extra.length > 0) {
			dataIn.read(this.extra);
		} else {
			this.extra = null;
		}
		if (dataIn.readBoolean()) {
			this.comment = dataIn.readUTF();
		}
		this.esize = dataIn.readLong();
	}
	...
	public Object clone() {
		try {
			WZipEntry e = (WZipEntry) super.clone();
